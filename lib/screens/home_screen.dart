...
class _HomeScreenState extends State<HomeScreen>
  ...
  final Set<String> _visibleShells = {
    'LEO',
    'MEO',
    'GEO',
    'Debris',
  ...
  void initState() {
    ...
    if (_visibleConstellations.length <
        constellation.constellationGroups.length) {
      debugPrint(
        'URL filter: constellations=${_visibleConstellations.join(",")}',
      );
    }
    if (_visibleShells.length < 6) {
      debugPrint(
        'URL filter: hidden shells=${const {'LEO', 'MEO', 'GEO', 'Debris', 'Station', 'Rocket-Body'}.difference(_visibleShells).join(",")}',
      );
    }
    ...
    debugPrint(
      'SpaceJunk: ${_allParticles.length} particles (LEO: $leoC, MEO: $meoC, GEO: $geoC, Debris: $debC, Station: $stnC)',
    );
  ...
  Future<void> _fetchLiveData({bool forceRefresh = false}) async {
    ...
    try {
      ...
      final debris = proceduralAll.where((p) => p.shell == 'Debris').toList();
      final stations = proceduralAll
          .where((p) => p.shell == 'Station')
          .toList();
  ...
  void _applyUrlParams() {
    ...
    if (cfg.constellations.isNotEmpty) {
      ...
      debugPrint(
        'URL filter: showing only constellations: ${cfg.constellations.join(", ")}',
      );
    }
  ...
  void _onScaleUpdate(ScaleUpdateDetails d) {
    if (_lastFocalPoint != null) {
      ...
      _targetRotationX = (_dragStartX + dy * 0.008).clamp(
        -pi / 2 + 0.1,
        pi / 2 - 0.1,
      );
    }
  ...
  Uri _buildShareUri() {
    ...
    final hiddenShells = _allShellIds.difference(_visibleShells).toList()
      ..sort();
            ...
            Container(
              ...
              child: Row(
                ...
                children: [
                  ...
                  const SizedBox(width: 6),
                  Text(
                    _formatDate(_historicalOffsetDays),
                    style: TextStyle(
                      fontSize: 11,
                  ...
                  const SizedBox(width: 6),
                  Text(
                    _historicalOffsetDays < 0 ? 'PAST' : 'FUTURE',
                    style: TextStyle(
                      fontSize: 8,
          ...
          Container(
            ...
            child: Row(
              ...
              children: [
                ...
                _divider(),
                _statItem(
                  _visibleShells.contains('LEO') ? '$_leoCount' : '—',
                  'LEO',
                  const Color(0xFFFF6B35),
                ),
                _divider(),
                _statItem(
                  _visibleShells.contains('MEO') ? '$_meoCount' : '—',
                  'MEO',
                  const Color(0xFFF7C948),
                ),
                _divider(),
                _statItem(
                  _visibleShells.contains('GEO') ? '$_geoCount' : '—',
                  'GEO',
                  const Color(0xFF4FC3F7),
                ),
                _divider(),
                _statItem(
                  _visibleShells.contains('Debris') ? '$_debrisCount' : '—',
                  'Debris',
                  const Color(0xFFEF5350),
                ),
                _divider(),
                _statItem(
                  _visibleShells.contains('Station') ? '$_stationCount' : '—',
                  'Stn',
                  const Color(0xFFFFD740),
                ),
                _divider(),
                _statItem(
                  _visibleShells.contains('Rocket-Body')
                      ? '$_rocketBodyCount'
                      : '—',
                  'RB',
                  const Color(0xFFC10A0A),
                ),
                const SizedBox(width: 8),
                GestureDetector(
  ...
  Widget _buildControlsHint() {
    return Positioned(
      ...
      child: AnimatedOpacity(
        ...
        child: Center(
          child: Text(
            'Tap any dot · Drag to orbit · Scroll to zoom',
            style: TextStyle(
              fontSize: 10,
              color: Colors.white.withValues(alpha: 0.12),
              letterSpacing: 1,
  ...
  // ------------------------------------------------------------------
  Widget _buildPopup() {
    if (_selectedParticle == null || _tapPosition == null)
    ...
    final satcat =
        p.satcat ?? (p.noradId > 0 ? _celestrak.getSatcat(p.noradId) : null);
    ...
    if (left < 10) left = 10;
    if (left > screenSize.width - popupWidth - 10)
          ...
          child: Column(
            ...
            children: [
              ...
              if (p.constellation != null)
                _popupRow(
                  'Constellation',
                  constellation.constellationLabel(p.constellation!),
                ),
    ...
    return Row(
      children: [
        ...
        Container(
          ...
          child: Text(
            satcat.isOperational
                ? 'ACTIVE'
                : satcat.decayDate.isNotEmpty
                ? 'DECAYED'
                : 'INACTIVE',
            style: TextStyle(
              fontSize: 7,
  ...
  Widget _buildTimeControls() {
    ...
    final direction = _historicalOffsetDays < 0
        ? 'PAST'
                ...
                Column(
                  ...
                  children: [
                    ...
                    _shellRow(
                      0xFFFF6B35,
                      'LEO',
                      '200–2,000 km — Imaging, ISS, Starlink',
                    ),
                    _shellRow(
                      0xFFF7C948,
                      'MEO',
                    ...
                    _shellRow(
                      0xFF4FC3F7,
                      'GEO',
                      '35,786 km — TV, weather, comms',
                    ),
                    _shellRow(
                      0xFFEF5350,
                      'Debris',
                      'untracked fragments <10 cm',
                    ),
                    _shellRow(
                      0xFFFFD740,
                      'Station',
                      'crewed outposts (ISS, Tiangong)',
                    ),
                  ],
                ),
                const SizedBox(height: 14),
  ...
  Widget _cDot(int hexColor) {
    return Container(
      ...
      decoration: BoxDecoration(shape: BoxShape.circle, color: Color(hexColor)),
    );
  }
  ...
  void _resetFilters() {
    setState(() {
      _visibleShells.clear();
      _visibleShells.addAll(_allShellIds);
      _visibleConstellations.clear();
      ...
      child: SingleChildScrollView(
        child: Column(
          ...
          children: [
            ...
            Row(
              children: [
                ...
                TextButton(
                  onPressed: () {
                    setState(() {
                      _visibleShells = Set.from(_allShellIds);
                      _visibleConstellations = Set.from(
                        constellation.constellationGroups.map((g) => g.id),
                      );
                    });
            ...
            const SizedBox(height: 4),
            Text(
              'Toggle layers on/off to explore each orbital region:',
              style: TextStyle(
                fontSize: 11,
                color: Colors.white.withValues(alpha: 0.35),
              ),
            ...
            _filterRow(
              ...
              'Rocket-Body',
              'Rocket Bodies',
              'Launch vehicles and spent stages',
              0,
            ),
  ...
  Widget _buildConstellationFilterSection() {
    return Column(
      ...
      children: [
        ...
        const SizedBox(height: 4),
        Text(
          'Toggle a group → isolate only those satellites on the globe:',
          style: TextStyle(
            fontSize: 11,
            color: Colors.white.withValues(alpha: 0.35),
          ),
